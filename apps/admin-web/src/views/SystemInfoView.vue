    ...
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      <div class="relative overflow-hidden rounded-xl border border-slate-200 bg-white px-6 py-5">
        ...
        <div class="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 class="bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 bg-clip-text text-2xl font-bold tracking-tight text-transparent">System Information</h1>
            <p class="mt-1 text-slate-500">Overview of the technologies powering this application.</p>
          </div>
        ...
        <div class="border-b border-slate-100 px-6 py-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-slate-500">High-level system architecture and patterns.</p>
            </div>
          </div>
        </div>
        <div class="p-6">
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        <article v-for="group in stack" :key="group.label" class="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div class="border-b border-slate-100 px-6 py-4">
            <div class="flex items-center gap-3">
              <div class="flex h-9 w-9 items-center justify-center rounded-lg" :class="group.color.bg">
                <component :is="group.icon" class="h-5 w-5" :class="group.color.text" />
              </div>
              <div>
                <h2 class="text-lg font-semibold">{{ group.label }}</h2>
                <p class="text-sm text-slate-500">{{ group.items.length }} {{ group.items.length === 1 ? 'technology' : 'technologies' }}</p>
              </div>
            </div>
          </div>
          <div class="divide-y divide-slate-100">
            <div v-for="item in group.items" :key="item.name" class="flex items-center justify-between px-6 py-3">
      ...
      <article class="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-6 py-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-semibold">Database Models</h2>
              <p class="text-sm text-slate-500">Prisma schema models and their purpose.</p>
            </div>
          </div>
        </div>
        <div class="p-6">
          <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            ]" :key="model.name" class="rounded-lg border border-slate-200 px-4 py-3">
